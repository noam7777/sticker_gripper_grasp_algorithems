function max_load = numerical_analisis_crazy_square(sO,l_init,s_tot,gamma,n_film,peal_vel_model,full_peal_animation,path_to_exp_csv,plot_all_streamlines,plot_optimal_sticker_state,sigma,n_seeds)
...
% crazy square filet:
    a=75; % length of half of the square edge
    R = 10; % filet radius
    dr =0.1; %the distance between two points, the shape resolution
    ...
    theta2 = 180:270;
    x2 = -a+R+R.*cosd(theta2);
    y2 = -a+R+R.*sind(theta2);
    
    x2_segment = -a+R:dr*10:-3/5*a -R*cosd(30);
    y2_segment = ones(size(x2_segment)).*-a;
    
    
    theta3 = 270:330;
    x3 = -3/5*a -R.*cosd(30)+R.*cosd(theta3);
    y3 = -a +R+R*sind(theta3);
    
    x3_end = -3/5*a -R*cosd(30)+R*cosd(330);
    y3_end = -a +R+R*sind(330);
    x4_start = -1/5*a+R.*cosd(30)+R.*cosd(150);
    y4_start = -23.04-R+R.*sind(150);
    l3 = ((x3_end-x4_start)^2+(y3_end-y4_start)^2)^0.5;
    ii = 0:(dr*10)/l3:1;
    x3_segment = x3_end + (x4_start-x3_end).*ii;
    y3_segment = y3_end + (y4_start-y3_end).*ii;
    
    theta4 = 150:-1:90;
    x4 = -1/5*a+R.*cosd(30)+R.*cosd(theta4);
    y4 = -23.04-R+R.*sind(theta4);
    
    x5 = -1/5*a+R*cosd(30)+ R*cosd(90):dr*10:-0.0000001;
    y5 = ones(size(x5)).*-23.04;
    
    x_q = [x1(2:end-1),x2,x2_segment(2:end-1),x3,x3_segment(2:end-1),x4,x5(2:end-1)];
    y_q = [y1(2:end-1),y2,y2_segment(2:end-1),y3,y3_segment(2:end-1),y4,y5(2:end-1)];
...
% n_seeds = 20;
% sigma = 2; % related to the distribution of the sticker's initial state at the grasp point
